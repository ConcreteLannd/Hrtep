...
// 管理者宛に送信されるメールのタイトル（件名） - ADMIN
$subject = "ランディングページより無料登録がありました。";
...
// 差出人に送信確認メールを送る場合のメールのタイトル（上記で1を設定した場合のみ）- USER
$re_subject = "無料登録ありがとうございます。";
...
if(isset($_POST)) {
  $postData = array();
  $postData["企業名"] = $_POST["企業名"];
  $postData["代表者名"] = $_POST["代表者名"];
  $postData["本社所在地"] = $_POST["本社所在地"];
  $postData["ご連絡の付く電話番号"] = $_POST["ご連絡の付く電話番号"];
  $postData["ご担当者名"] = $_POST["ご担当者名"];
  $postData["ご担当者様メールアドレス"] = $_POST["ご担当者様メールアドレス"];
  $postData["所属部署名"] = $_POST["所属部署名"];
...
if(($confirmDsp == 0 || $sendmail == 1) && $empty_flag != 1){
  ...
  // CSV出力
  // 必要項目をセット
  $csv_export_dir_path  = __DIR__ .'/csv/databox/';// 出力先ディレクトリパス
  $csv_export_file_path = __DIR__ .'/csv/databox/data.csv';// 出力先ファイルパス
  $column_names  = [];// csv一行目の配列(列タイトル)
  $column_values = [];// csv出力対象データ
  foreach ($_POST as $k => $v) {
      if ($k != 'mail_set' && $k != 'httpReferer') {//除外リスト
          $column_names[$k]  = h($k);
          $column_values[$k] = h($v);
      }
      if ($k == '電話番号') {
        $column_values['電話番号'] = '[' . h($v) . ']';
      }
  }
  $date = date("Y/m/d");
  ...
  $dy   = $week[date("w")];
  $dat = $date."(".$dy.")"." ".$time;
  array_push($column_names, '送信された日時');
  array_push($column_values, $dat);

  // 処理の呼び出し
  //   include_once __DIR__ . '/csv/CsvController.php';
  //   $Csv = new CsvController($csv_export_dir_path, $csv_export_file_path, $column_names);
  //   $Csv->insert($column_values);

  /* Send mail width file */
...
function mailToAdmin($arr,$subject,$mailFooterDsp,$mailSignature,$encode,$confirmDsp){
	$adminBody="ランディングページより下記内容で登録がありました。\n";
